  ...
  class Rules
    ...
    class << self
      ...
      def build_rule rule
        o = build_rule_entry
        if o.length == 3
          # i.e all of :namespace, :controller, :action
          n = 'namespace-'+@_params[:namespace][:name].to_s
          c = ['controller-'+@_params[:controller][:name].to_s]
          a = ['action-'+@_params[:controller][:name].to_s]
          
          rules[n] ||= {}
          if @_params[:namespace][:only] and @_params[:namespace][:only].length
          ...
          # i.e any of :namespace, :controller, :action
        else
          raise InvalidSyntax.new('this should never happen!');
        end
      end
